...
import { ShoppingCart, ArrowLeft, MessageSquare, Star, ShieldCheck, Truck, RotateCcw } from 'lucide-react';
...
export default function ProductDetail() {
  ...
  if (loading) return <div className="mt-20 text-center"><div className="w-16 h-16 border-4 border-blue-600 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div><p className="text-slate-400 animate-pulse font-medium">Entering the elite experience...</p></div>;
  if (!product) return <div className="mt-20 text-center py-20 bg-slate-900/50 rounded-3xl max-w-2xl mx-auto border border-slate-800"><h2 className="text-2xl font-bold text-white mb-2">Item not found</h2><p className="text-slate-500 mb-6">The product you're looking for might have been moved or deleted.</p><Link href="/" className="bg-blue-600 text-white px-6 py-2 rounded-xl font-bold">Return Home</Link></div>;
  ...
  return (
    <div className="mt-8 max-w-7xl mx-auto px-4 pb-20">
      <Link href="/" className="inline-flex items-center gap-2 text-slate-400 hover:text-white transition-all mb-10 group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="premium-card rounded-[2.5rem] p-10 relative overflow-hidden flex items-center justify-center min-h-[500px]">
            ...
            <div className="w-full max-w-[450px] aspect-square flex items-center justify-center relative group">
              {product.images?.length > 0 ? (
                ...
                <div className="w-full h-full bg-slate-900/50 rounded-3xl border border-slate-800 flex items-center justify-center italic text-slate-700 font-medium">No Image Available</div>
              )}
            </div>
          </div>
            ...
            <div className="flex gap-4 overflow-x-auto py-2 no-scrollbar scroll-smooth">
              {product.images.map((img: any, idx: number) => (
                <button 
                  ...
                  className={`relative shrink-0 w-24 h-24 p-2 rounded-2xl bg-slate-900 border-2 transition-all overflow-hidden ${activeImg === idx ? 'border-blue-500 ring-4 ring-blue-500/10' : 'border-slate-800 hover:border-slate-700 opacity-70 hover:opacity-100'}`}
                >
        ...
        <div className="lg:col-span-5 space-y-8">
          <div className="space-y-4">
             ...
             <h1 className="text-5xl font-black text-white leading-tight tracking-tight">{product.name}</h1>
             <div className="flex items-center justify-between border-b border-slate-800 pb-6 mb-6">
                ...
                <div className="text-right">
                   ...
                   <p className={`font-bold ${product.stock > 0 ? "text-emerald-400" : "text-rose-500"}`}>
          ...
          <div className="relative">
             <h3 className="text-xs font-black uppercase text-slate-500 mb-3 tracking-widest">Description</h3>
          ...
          <div className="premium-card rounded-[2rem] p-8 space-y-6">
             <div className="flex justify-between items-end">
                <div>
                   <p className="text-slate-400 text-sm font-medium mb-1 uppercase tracking-widest">Total Value</p>
             ...
             <button 
               ...
               className={`group w-full h-16 rounded-2xl flex items-center justify-center gap-3 transition-all text-xl font-black shadow-2xl ${product.stock === 0 ? 'bg-slate-800 text-slate-600 grayscale cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-500 shadow-blue-600/30 hover:scale-[1.01] active:scale-95'}`}
             >
               <ShoppingCart size={24} className={product.stock > 0 ? "group-hover:rotate-12 transition-transform" : ""} /> 
             ...
             <div className="grid grid-cols-3 gap-2 pt-2">
                <div className="flex flex-col items-center p-3 rounded-xl border border-slate-800/50 bg-slate-900/30">
                   ...
                   <span className="text-[9px] font-black uppercase tracking-tighter text-slate-500">Warranty</span>
                </div>
                <div className="flex flex-col items-center p-3 rounded-xl border border-slate-800/50 bg-slate-900/30">
                   <Truck size={18} className="text-blue-400 mb-2" />
                   <span className="text-[9px] font-black uppercase tracking-tighter text-slate-500">Fast Shipping</span>
                </div>
                <div className="flex flex-col items-center p-3 rounded-xl border border-slate-800/50 bg-slate-900/30">
                   <RotateCcw size={18} className="text-blue-400 mb-2" />
                   <span className="text-[9px] font-black uppercase tracking-tighter text-slate-500">Returns</span>
                </div>
             </div>
          </div>
        </div>
      ...
      <div className="mt-24 space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {product.comments?.length > 0 ? product.comments.map((c: any) => (
             <div key={c.id} className="premium-card rounded-3xl p-8 hover:border-blue-500/30 transition-colors group">
               <div className="flex justify-between items-start mb-6">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       ...
                       <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{new Date(c.created_at).toLocaleDateString()}</span>
                    </div>
                 </div>
                 <div className="flex text-blue-400">
                    {[1,2,3,4,5].map(i => <Star key={i} size={12} className="fill-blue-400" />)}
                 </div>
               </div>
               <p className="text-slate-400 leading-relaxed font-medium italic group-hover:text-slate-300 transition-colors">"{c.text}"</p>
             </div>
          )) : (
        ...
        <div className="max-w-3xl">
          {token ? (
            ...
            <div className="premium-card rounded-[2.5rem] p-12 mt-12 text-center border-dashed border-blue-500/20">
              <p className="text-slate-400 text-lg mb-8 font-medium">Join the community to post your own reviews.</p>
              <Link href="/login" className="bg-blue-600 hover:bg-blue-500 text-white px-12 py-4 rounded-2xl font-black text-xl transition-all shadow-xl shadow-blue-600/40">
